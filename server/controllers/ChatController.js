...
//controller  for geting chat(creating if not exist)
    ...
    try {
        ...
        if(existingChat){
            ...
            if(existingChat.isLastMessageRead===false){
                const lastMassage = existingChat.messages[existingChat.messages.length -1]
                const isLastMessageSendByMe = lastMassage.sender_id===userId;
...
export const sendChatMessage = async(req,res)=>{
    try {
        ...
        res.json({message:"Message Sent",newMessages:newMessage})
    } catch (error) {
        console.log(error)
        res.status(500).json({message:error.code||error.message})
    }
