...
import { CheckCircle2, ChevronDown, ChevronUp, Copy, DollarSignIcon, Loader2Icon } from 'lucide-react'
...
if(loading){
  return (
    <div className='h-[80-vh] flex items-center justify-center'>
...
if(!orders.length){
  return(
    <div className='px-4 md:px-16 lg:px-24 xl:px-32 '>
      <div className='max-w-2xl mx-auto mt-14 bg-white rounded-xl border border-gray-300 text-center p-8'>
      <h3 className='text-lg font-semibold'>No Orders yet</h3>
      <p className='text-sm text-gray-500 mt-2'>you haven't puchased any listings yet.</p>
      </div>
          ...
          return(
            <div key={id} className='bg-white rounded-lg border border-gray-200 p-5 flex flex-col max-w-4xl'>
                ...
                <div className='flex gap-2 mt-2'>
                  {
                    listing.verified &&(
                      <span className='flex items-center text-xs bg-indigo-50 text-indigo-600 px-2 py-1 rounded-md'>
                        <CheckCircle2 className='w-3 h-3 mr-1'/>Verifiied
                      </span>
                    )
                  }
                   {
                    listing.monetized &&(
                      <span className='flex items-center text-xs bg-green-50 text-green-600 px-2 py-1 rounded-md'>
                        <DollarSignIcon className='w-3 h-3 mr-1'/>Monetized
                      </span>
                    )
                  }
              ...
              <div className='flex flex-col gap-0 items-end'>
                ...
                className='flex items-center gap-2 bg-white border border-gray-200 px-3 py2 rounded hover:shadow text-sm '>
                  {isExpanded?(
                    <>
                      ...
                      credential.updatedCredential.map((cred)=>(
                        <div key={cred.name} className='flex items-center justify-between gap-3 bg-gray-50 rounded-md p-2'>
                          ...
                          <div className='flex items-center gap-2'>
                            ...
                            <button onClick={(e)=>{e.stopPropagation();copy(cred.value)}} 
