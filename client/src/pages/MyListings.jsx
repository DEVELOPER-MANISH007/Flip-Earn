    ...
    <div className="px-6 md:px-16 lg:px-24 xl:px-32 pt-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
        <div>
          ...
          <p className="text-gray-600 mt-1">Manage your social media acccount listings</p>
        </div>
      ...
      <div className="grid  grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        ...
        <StatCard value={soldListings} title="Sold" icon={<TrendingUp className="size-6 text-indigo-600" />} color="indigo" />

        <StatCard value={`${currency}${totalvalue.toLocaleString()}`} title="Total Value" icon={<DollarSign className="size-6 text-indigo-600" />} color="yellow" />
        ...
        {[
          ...
          <div onClick={()=>item.label==="Available" && setShowWithdarwal(true)}  key={index} className="flex flex-1 items-center justify-between p-4 rounded-lg border border-gray-100 cursor-pointer">
            <div className=" flex items-center gap-3">
              ...
              <span className=" font-medium to-gray-600">{item.label}</span>
            </div>
          ...
          (
            <div className="bg-white rounded-lg border border-gray-200 p-16 text-center">
              <div className="w-16 h-16 bg-gray-100 rounded-full flex items-center justify-center mx-auto mb-4">
                <Plus className="w-8 h-8 text-shadow-gray-400" />
              </div>
                ...
                <div className="p-6">
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-2 text-sm">
                      ...
                      <span className={`flex items-center gap-1 justify-end ${getStatusColor(listing.status)}`}>
                        {getStatusIocn(listing.status)}{" "} <span>{listing.status}</span>
                      </span>
                      <div className="flex items-center space-x-2">
                    ...
                    <div className="flex items-center justify-between pt-3 border-t border-gray-200">
                          ...
                          <div className="flex items-center space-x-">
                            {listing.status!=="sold" &&(
                              <button onClick={()=>deleteListing(listing.id)} className="p-2 border border-gray-300 rounded-lg hover:bg-gray-500 hover:text-red-50">
      ...
      {showWithdarwal && (
        <WithdrawModal onClose={()=>setShowWithdarwal(null)}/>
      )}
      {/* footer */}
